...
森ビル コーポレートサイト・ヒルズ各施設サイト（麻布台ヒルズ等）のビジュアル言語を参考に、
ブランドイメージを統一したオリジナルのデザインシステムで構成しています。

- **配色** … アイボリーの余白を基調に、深いグリーン・チャコール（ヒルズの品格）、
  自然・ウェルネスのシグネチャーグリーン、上質なブロンズのアクセント。
- **書体** … ディスプレイに明朝（Shippori Mincho）、本文に Noto Sans JP。建築的な余白と字間。
- **素材** … 第三者の著作物（ロゴ・写真）は同梱していません。画像はCSSによるプレースホルダ表現で、
  Web フォントは Google Fonts から配信。実運用時は森ビルから正式に許諾された素材へ差し替えてください。

デザイントークンは `tailwind.config.js`、ベーススタイルは `src/index.css`、
共通UI部品は `src/hills/ui.tsx` に集約しています。
