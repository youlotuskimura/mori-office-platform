...
import { customers, deals, Deal, TimelineEvent, facilities } from './data'

const STAGE_MAP: Record<Deal['stage'], { label: string; color: string; step: number }> = {
  inquiry:     { label: '問い合わせ', color: 'bg-stone-100 text-stone-600',   step: 1 },
  proposal:    { label: '提案中',    color: 'bg-blue-50 text-blue-700',       step: 2 },
  tour:        { label: '見学済み',  color: 'bg-amber-50 text-amber-700',     step: 3 },
  negotiation: { label: '条件調整',  color: 'bg-purple-50 text-purple-700',   step: 4 },
  booked:      { label: '予約確定',  color: 'bg-emerald-50 text-emerald-700', step: 5 },
}
...
export default function CRM() {
  ...
  const addLog = (type: TimelineEvent['type'], title: string, detail: string) => {
    if (!deal) return
  ...
  const advanceStage = () => {
    if (!deal) return
    const stages: Deal['stage'][] = ['inquiry', 'proposal', 'tour', 'negotiation', 'booked']
          ...
          ? {
              ...
              timeline: [
                ...d.timeline,
                {
                  date: new Date().toISOString().split('T')[0],
                  type: nextStage,
                  title: `ステージ更新: ${STAGE_MAP[nextStage].label}`,
                  detail: 'ステータスを次フェーズに進めました。（デモ）',
                },
              ],
            }
          : d
    ...
    <div className="flex h-full flex-col md:flex-row md:divide-x md:divide-stone-200">
      ...
      <div className="shrink-0 overflow-y-auto md:w-64">
        <div className="border-b border-stone-200 bg-stone-50 px-4 py-2.5">
          <p className="text-xs font-semibold text-stone-500">担当案件 {customers.length}件</p>
        </div>
              ...
              <li key={c.id}>
                <button
                  ...
                  className={`w-full px-4 py-3 text-left transition-colors hover:bg-stone-50 ${
                    selectedCustomerId === c.id ? 'border-r-2 border-ink-900 bg-ink-900/5' : ''
                  }`}
                >
                  <div className="flex items-start justify-between gap-1.5">
                    <p className="text-sm font-semibold text-ink-900">{c.name}</p>
                    {d && (
                      <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold ${STAGE_MAP[d.stage].color}`}>
                        {STAGE_MAP[d.stage].label}
                      </span>
                    )}
                  </div>
                  <p className="text-[11px] text-stone-500 truncate">{c.company}</p>
                  <p className="mt-0.5 text-[10px] text-stone-400">{c.requirements.occasion}</p>
                  {d && (
                    <div className="mt-1.5">
                      <div className="h-1 w-full overflow-hidden rounded-full bg-stone-200">
                        <div
                          className={`h-full rounded-full ${d.probability >= 75 ? 'bg-emerald-400' : d.probability >= 50 ? 'bg-amber-400' : 'bg-stone-300'}`}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="border-b border-stone-200 bg-white px-4 py-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-stone-400">{customer.industry} / 問い合わせ: {customer.inquiryDate}</p>
            </div>
            {deal && (
          ...
          <div className="mt-3 rounded-lg bg-stone-50 p-3">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 sm:grid-cols-3">
              <ReqItem label="用途" value={customer.requirements.occasion} />
              <ReqItem label="人数" value={`${customer.requirements.headcount}名`} />
              <ReqItem label="予算" value={customer.requirements.budgetNote} />
              <ReqItem label="希望日程" value={customer.requirements.dateNote} />
              <ReqItem label="ヒルズ" value={customer.requirements.preferredHills.join('・')} />
              <ReqItem label="こだわり" value={customer.requirements.priorities.slice(0, 2).join('・')} />
            </div>
          </div>
        </div>

        {/* 関連施設 */}
        {deal && deal.relatedFacilities.length > 0 && (
          <div className="border-b border-stone-200 bg-stone-50 px-4 py-3">
            <p className="mb-2 text-xs font-semibold text-stone-500">提案中の施設</p>
            <div className="flex flex-wrap gap-2">
              {deal.relatedFacilities.map(id => {
                const f = facilities.find(x => x.id === id)
                if (!f) return null
                return (
                  <span key={id} className="rounded-full border border-stone-200 bg-white px-2.5 py-1 text-xs font-medium text-stone-700">
                    {f.name}
                  </span>
                )
              })}
        ...
        {deal && (
          <div className="border-b border-stone-200 bg-white px-4 py-3">
            <div className="flex items-center gap-1">
              {(['inquiry', 'proposal', 'tour', 'negotiation', 'booked'] as Deal['stage'][]).map((stage, i, arr) => {
                ...
                return (
                  <React.Fragment key={stage}>
                    <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold transition-colors ${
                      thisStep <= current ? 'bg-ink-900 text-white' : 'bg-stone-200 text-stone-400'
                    }`}>
                      {thisStep <= current ? '✓' : thisStep}
                    </div>
                    {i < arr.length - 1 && (
                      <div className={`h-0.5 flex-1 rounded-full transition-colors ${thisStep < current ? 'bg-ink-900' : 'bg-stone-200'}`} />
                    )}
                  </React.Fragment>
                )
              })}
            ...
            <div className="mt-1.5 flex justify-between text-[10px] text-stone-400">
              <span>問い合わせ</span><span>提案</span><span>見学</span><span>条件調整</span><span>確定</span>
            </div>
          </div>
        )}
          ...
          <div className="border-b border-stone-200 bg-stone-50 px-4 py-2.5">
            ...
            {showNoteInput && (
              <div className="mt-2 flex gap-2">
                ...
                />
                <button
                  onClick={() => { addLog('note', '営業メモ', noteText); setNoteText(''); setShowNoteInput(false) }}
        ...
        {deal ? (
          <div className="px-4 py-4">
            <h3 className="mb-4 text-xs font-semibold uppercase tracking-wide text-stone-500">商談タイムライン</h3>
